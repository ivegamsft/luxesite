          ...
          {destinations.map((destination, index) => {
            ...
            return (
              <motion.div
                ...
                transition={prefersReducedMotion ? { duration: 0 } : { duration: 0.6, ease: [0.22, 1, 0.36, 1] }}                className={`destination-card group relative rounded-sm overflow-hidden border border-aurora-border shadow-subtle transition-all duration-300 ease-out hover:-translate-y-1 hover:shadow-medium ${isFeature ? 'md:col-span-2 md:row-span-2' : ''}`}
                role="button"
                tabIndex={0}
              ...
              >
                <div className={`relative ${isFeature ? 'aspect-[16/9] md:aspect-auto md:h-full' : 'aspect-[4/3]'}`}>
                  ...
                  <div className={`absolute inset-0 bg-black/95 transition-opacity duration-300 p-6 flex flex-col justify-center group-hover:opacity-100 focus-within:opacity-100 ${isExpanded ? 'opacity-100' : 'opacity-0'}`}>
